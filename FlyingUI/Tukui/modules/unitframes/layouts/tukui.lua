...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		if T.lowversion then
			panel:CreatePanel("Invisible", 186, 21, "BOTTOM", self, "BOTTOM", 0, 0)
		else
			panel:CreatePanel("Invisible", 200, 35, "BOTTOM", self, "BOTTOM", 0, 6)
		end
		panel:SetFrameLevel(2)
		panel:SetFrameStrata("MEDIUM")
		panel:SetTemplate("Invisible")
		panel:SetAlpha(0)
		self.panel = panel
		...
		if C["unitframes"].unicolor == true then
			...
			health:SetStatusBarColor(.18, .18, .18, 1)
		...
		if (unit == "player") then
			...
			if T.level ~= MAX_PLAYER_LEVEL then
				...
				Experience:Width(139)
				Experience:Height(4)
				Experience:Point("TOPLEFT", expb, 4, -4)
				Experience:Point("BOTTOMRIGHT", expb, -4, 4)
			...
			if T.level == MAX_PLAYER_LEVEL then
				...
				Reputation:Width(139)
				Reputation:Height(4)
				Reputation:Point("TOPLEFT", expb, 2, -2)
				Reputation:Point("BOTTOMRIGHT", expb, -2, 2)
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" then			
					...
					eclipseBar:Point("BOTTOM", TukuiBar1, "BOTTOM", 0, -4)
					if T.lowversion then
						eclipseBar:Size(186, 8)
					else
						eclipseBar:Size(180, 5)
					end
				...
				if (T.myclass == "WARLOCK" or T.myclass == "PALADIN") then
					...
					bars:SetPoint("CENTER", health, "CENTER", 0, 0)
					if T.lowversion then
						bars:Width(186)
					else
						bars:Width(15)
					...
					bars:Height(13)
					bars:SetTemplate("Invisible")
					bars:SetBackdropBorderColor(0,0,0,0)
					bars:SetFrameLevel(9)
					bars:SetFrameStrata("MEDIUM")
					
					-- Soul border
					local soulborder = CreateFrame("Frame", nil, bars)
					soulborder:CreatePanel("Invisible", 1, 1, "CENTER", bars, "CENTER", 0, 0)
					soulborder:ClearAllPoints()
					soulborder:Point("TOPLEFT", bars, -2, 2)
					soulborder:Point("BOTTOMRIGHT", bars, 2, -2)
					...
					for i = 1, 3 do					
						...
						if T.myclass == "WARLOCK" then
							bars[i]:SetStatusBarColor(200/255,50/255,200/255)
							bars[i].bg:SetTexture(255/255,255/255,101/255)
						...
						if i == 1 then
							...
							if T.lowversion then
								...
								bars[i]:Width(5) -- setting SetWidth here just to fit fit 250 perfectly
							end
				...
				if T.myclass == "DEATHKNIGHT" then
					...
					Runes:Point("CENTER", health, "CENTER", 0, -13)
					Runes:Height(17)
					if T.lowversion then
						Runes:SetWidth(186)
					else
					...
					for i = 1, 6 do
						...
						if (i == 1) then
							Runes[i]:Point("BOTTOMLEFT", Runes, "TOPLEFT", 0, -3)
						else
							Runes[i]:Point("TOPLEFT", Runes[i-1], "TOPRIGHT", 0, 0)
						end
				...
				if T.myclass == "SHAMAN" then
					...
					Totems:Point("CENTER", self, "CENTER", 0, 0)
					Totems:Height(17)
					Totems:SetWidth(17)
					Totems:SetTemplate("Invisible")
					...
					for i = 1, 4 do
						...
						if (i == 1) then
						   TotemBar[i]:Point("TOPLEFT", Totems, "TOPLEFT", 0, 5)
						else
						   TotemBar[i]:Point("TOPLEFT", TotemBar[i-1], "TOPRIGHT", 0, 0)
						end
						TotemBar[i]:SetStatusBarTexture(normTex)
						TotemBar[i]:Height(17)
						if T.lowversion then
							if i == 1 then
								TotemBar[i]:SetWidth(45)
							else
								TotemBar[i]:SetWidth(5)
							end
						else
							if i == 4 then
								TotemBar[i]:SetWidth(5)
		...
		if (unit == "target" and C["unitframes"].targetauras) or (unit == "player" and C["unitframes"].playerauras) then
			...
			if (T.myclass == "SHAMAN" or T.myclass == "DEATHKNIGHT" or T.myclass == "PALADIN" or T.myclass == "WARLOCK") and (C["unitframes"].playerauras) and (unit == "player") then
				...
				if T.lowversion then
					buffs:SetPoint("TOPLEFT", self, "TOPLEFT", -4, 26)
				else
					buffs:SetPoint("TOPLEFT", self, "TOPLEFT", -4, 35)
				end
			end
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if unit == ("player") then
			castbar:Point("BOTTOM", UIParent, "BOTTOM", 0, 68)
			elseif unit == ("target") then
			castbar:Point("BOTTOM", UIParent, "BOTTOM", 0, 348)
			end
			...
			--------- This will be a looooong line for my custom Class Border. :)
			-- health border one
			local castborder = CreateFrame("Frame", nil, castbar)
			castborder:CreatePanel("Thin", 1, 1, "CENTER", castbar, "CENTER", 0, 0)
			castborder:ClearAllPoints()
			castborder:Point("TOPLEFT", castbar, -1, 1)
			castborder:Point("BOTTOMRIGHT", castbar, 1, -1)
	...
	if (unit == "targettarget") then
		...
		if not T.lowversion then
			panel:CreatePanel("Invisible", 129, 17, "BOTTOM", self, "BOTTOM", 0, T.Scale(0))
			panel:SetFrameLevel(2)
			panel:SetAlpha(0)
			panel:SetFrameStrata("MEDIUM")
...
tot:SetTemplate("Invisible")
if T.lowversion then
	tot:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", 0,8)
	tot:Size(186, 18)
else
	tot:SetPoint("BOTTOM", InvTukuiActionBarBackground, "TOP", 0,12)
	tot:Size(100, 22)
end

-- pet
local pet = oUF:Spawn('pet', "TukuiPet")
pet:SetTemplate("Invisible")
if T.lowversion then
...
if C["unitframes"].showboss then
	...
	for i = 1, MAX_BOSS_FRAMES do
		...
		if i == 1 then
			boss[i]:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", 200 - adjust, 246)
		else
			boss[i]:SetPoint('BOTTOM', boss[i-1], 'TOP', 0, 35)             
		end
		boss[i]:Size(200, 29)
		boss[i]:SetTemplate("Invisible")
	end
end
