...
T.PP = function(p, obj)
	...
	elseif p == 3 then
		obj:SetParent(left)
		obj:SetHeight(left:GetHeight())
		obj:SetPoint("RIGHT", left, -4, 0)
		obj:SetPoint('TOP', left)
		obj:SetPoint('BOTTOM', left)
	elseif p == 4 then
		obj:SetParent(right)
		obj:SetHeight(right:GetHeight())
		obj:SetPoint("LEFT", right, 4, 0)
		...
		if min ~= max then
			...
			if unit == "player" and health:GetAttribute("normalUnit") ~= "pet" then
				if C["unitframes"].showtotalhpmp == true then
					...
					health.value:SetFormattedText("|cffAF5050%d|r", min, r * 255, g * 255, b * 255, floor(min / max * 100))
				end
			elseif unit == "target" or (unit and unit:find("boss%d")) then
				if C["unitframes"].showtotalhpmp == true then
					health.value:SetFormattedText("|cff559655%s|r |cffD7BEA5|||r |cff559655%s|r", ShortValue(min), ShortValue(max))
				else
					health.value:SetFormattedText("|cffAF5050%s|r", ShortValue(min), r * 255, g * 255, b * 255, floor(min / max * 100))
				end
			...
			if pType == 0 then
				if unit == "target" then
					if C["unitframes"].showtotalhpmp == true then
						...
						power.value:SetFormattedText("%d%%", floor(min / max * 100), ShortValue(max - (max - min)))
					end
				...
				elseif (unit and unit:find("arena%d")) or unit == "focus" or unit == "focustarget" then
					...
					if C["unitframes"].showtotalhpmp == true then
						...
						power.value:SetFormattedText("%s%%", floor(min / max * 100), max - (max - min))
					end
				end
			else
